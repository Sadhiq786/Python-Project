...
class Attendance:
    ...
    def __init__(self,root):
        ...
        #title section
        title_lb1 = Label(bg_img,text="Welcome to Attendance Panel",font=("verdana",25,"bold"),bg="white",fg="navyblue")
        ...
        #Student dep
        studentId_label = Label(left_frame,text="Department:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        studentId_label.grid(row=0,column=0,padx=5,pady=5,sticky=W)

        studentId_entry = ttk.Entry(left_frame,textvariable=self.var_dep,width=15,font=("verdana",12,"bold"))
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,column=("Department","Roll_No","Name","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        #create table 
        self.attendanceReport = ttk.Treeview(table_frame,column=("Department","Roll_No","Name","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    def update_data(self):
        if self.var_dep.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update stdattendance set Department=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where Department=%s",( 
                    self.var_dep.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
        ...
        if self.var_dep.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(username='root', password='project',host='localhost',database='face_recognition',port=3306)
                    mycursor = conn.cursor() 
                    sql="delete from stdattendance where Department=%s"
